...
from core.bigquery import BQ_PROJECT, BQ_DATASET, BQ_TABLE_COMPAT, BQ_TABLE_USERS, get_bq_client
from core.hashing import make_user_id
import sqlalchemy
...
async def get_worth_matches(
    ...
    # 1. Get current user's name to compute their BigQuery user_id
    row = db.execute(
        sqlalchemy.text(
            "SELECT first_name, last_name FROM users WHERE firebase_uid = :uid"
        ),
        {"uid": user["uid"]},
    ).fetchone()
    if not row:
    ...
    # 2. Query BigQuery — join compat_results with users_processed for matched user info
...
async def get_worth_profile(
    ...
    """Return a single user's profile from BigQuery users_processed."""
    users_table = f"`{BQ_PROJECT}.{BQ_DATASET}.{BQ_TABLE_USERS}`"
